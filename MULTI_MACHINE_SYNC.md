...
### 腳本更新
- AncTestScript.kt：
  - 延伸改進 `CarRoadTuningScript`（SCRIPT_ID = "car_road_tuning_v1"），基於#4 強制低延遲 + musicLow 對比（Skoda 200-350Hz rumble 專用，override=120）。
  - 新增 tuning_4b_Skoda 步驟（override=150, mu=1.6 mid-focus），總步驟延伸為 prep+1-4-4b-5+finish。
  - 每個 tuning step 自動套用 muMult / freezeThreshold / freezeConsec / latencyOverride + forceNormal / musicLowAnc / userAncGain（#4b 為 #4 延伸）。
  - 腳本開頭/ prep/ finish instructions 更新：強調計劃 3 次完整迭代 + 配外部錄音 + spectrum，優先觀察 midBand 貢獻與 200-350Hz reduction。scenario 註 "Skoda #4/#4b 經驗, iter X"。比較 effective maxCancel 與 rumble 有感。
