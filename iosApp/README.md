...
| `version.properties` | Android 版號 | 目前 **1.2.40 / code 42** · 主畫面 `v1.2.40` |
| `iosApp/.../Info.plist` + Xcode | iOS 版號 | 原始碼 **1.2.40 (37)** · `dist/*.ipa` 仍是 **1.2.32 (33)**（未重編） |
...
| 能力 | iOS 現況 | Android |
|------|----------|---------|
| 版號顯示 | 原始碼 `v1.2.40 (37)`；**IPA 仍 `v1.2.32 (33)`** | ✅ 主畫面 `v1.2.40` |
| 無車速備用 | ✅ gps_hold + imu_proxy | ✅ 同 `VehicleSpeedFusion` |
| 多頻段 DSP | ✅ **KMP `MultiBandANCProcessor`** + SpeedScheduled | ✅ 同核心 |
| `speedNvh*` / boom / antiE / mute / 極性 / 真LF / openBoom / live-tune / 追 D | ✅ KMP 1.2.40 原始碼；**IPA 未帶上** | ✅ 1.2.40 |
...
| 預編 IPA | ⚠️ `dist/…ipa` 仍是 **v1.2.32 (33)**；要 1.2.40 須重編 | — |
...
1. 打開 App → **狀態** 頁右上角 / 副標：現裝 IPA 仍是 **`v1.2.32 (33)`**。原始碼已 **1.2.40 (37)**，**未重編 IPA 就沒有追 D**（**勿裝 build 22**）  
2. Session log 開頭：現裝 IPA `appVersion=1.2.32` `build=33`  
