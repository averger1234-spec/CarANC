    ...
    val steps: List<TestScriptStep> = listOf(
        ...
        TestScriptStep(
            ...
            instructions = listOf(
                ...
                "今日 log (07-01) 分析後續方向：phone 已用最新 APK 更新（包含 2.8x/3.5x + EMA + 0.25f 門檻 + 0.18f mic + accel>0.8 自動 mode + 連續 energyProxy 依 rumble 強度 scale）。重跑 strict spd>55 + low music + 故意觸發 notif，驗證 boost 平滑 sustained >2.5、sonif 期間 rumble boost 不中斷、reduction 改善。今日 log 顯示 mode 進入 + sonif 保護好，但 speed 低導致 boost 未充分 + red 低；新 code + 高 spd 應解決。若仍 peaky，可再調 EMA 權重或 continuous boost 函數 (f(accel))。 sub-agent C18 已加到 sim_iter.ps1（用此 log stats 校正，預測 lowsupp 保守保護、高supp 大gain）。 新增改善： sonif 更不干擾 (rumble muScale 不乘 sonif eventScale，只 duck 最終輸出)；virtualSuppressionQuality = max(media quality, rumbleEnergyProxy from accel) 用來在 quality=0 時仍 aggressive rumble。"
            ),
            durationSec = 75,
