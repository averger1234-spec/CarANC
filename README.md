...
**版號**：Android **`version.properties` → 1.2.13 / code 15** · iOS **1.2.13 (22)** · 改版見 **[`CHANGELOG.md`](CHANGELOG.md)**  
...
## 最新進度（2026-08-19）— 1.2.13 真 LF 終端 + 停中頻加噪
...
| **iOS 版號** | **`1.2.13` (build 22)** · 狀態頁 **`v1.2.13 (22)`** · 真LF+mute+極性A/B+艙錄+通話音量修復 | ✅ |
...
```bash
git pull origin main
# Android：.\scripts\install-debug.ps1  → 主畫面看 v1.2.13
# iOS：見 iosApp/README.md 或 dist/*.ipa → 狀態頁看 v1.2.13 (22)
```
...
## 最新進度（2026-07-22）— 請 pull 後讀這段

| 編號 | 內容 | 狀態 |
|------|------|------|
| **A** | wireless 誤判修正：USB 投影 `remote_submix` → `aaLinkType=projection_submix`，僅 BT 才 `wirelessAaSuspected` | 已做 |
| **B** | bank 真輸出：default prior + seed + capture；行駛 `fixedBankOut` 應非零 | 已做 |
| **C** | `reductionDb` 誠實（可負）；主 KPI=`lowBandRumbleReduction`；`reductionDbLegacy` 對照 | 已做 |
| **行駛白噪** | FF 關 mid/high、final lowpass、減 preview/FDAF 增益、bank 為主 | 已做 |
| P0–#11 | 見下表歷史 | 已做 |

**路測**：USB AA；怠速應靜；行駛聽沙沙是否下降；log：`aaLinkType` / `fixedBankOut` / `lowBandRumbleReduction` / `effectiveMidMu≈0`（高 lat）。  
**安裝**：`.\scripts\install-debug.ps1`  
**跨機器文件**：`GROK_RESUME_CONTEXT.md` + `README.md` + `MULTI_MACHINE_SYNC.md` 三份均含 2026-07-22 進度。

### 2026-07-21 歷史（P0 + #6–#9 + #11）

| 編號 | 內容 |
|------|------|
| P0 | `FF_PREVIEW_ONLY`、plant=measured、ov 僅 log |
| #6–#9 | delayless FDAF、2D bank、本機 AAudio-like、有線偏好 |
| #11 | Car App scaffold（非 OEM ECU） |  
**無車測真 AA（電腦當車機）**：`.\scripts\start-dhu.ps1`（Desktop Head Unit；詳見 `MULTI_MACHINE_SYNC.md` / `scripts/README.md`）。**不是** App 內假 AA 開關。

---

## 功能概覽

- **多頻帶主動降噪**：低／中／高頻 FxLMS，主攻 80–300 Hz 引擎與底噪；高延遲 AA 時 low 走 **FF_PREVIEW + fixed bank + delayless FDAF**
- **延遲感知頻帶控制**：依 **measured** 延遲調整可抵消頻率與頻帶增益（不吃 debug latencyOverride 當 plant）
- **車廂聲學校正**：對數掃頻（log chirp）估計 secondary path，支援 profile 儲存與老化重校
- **情境模式**：怠速、GPS 路噪、音樂播放 bypass、通話語音保護、MUSIC_DOMINANT_RUMBLE / driving rumble
- **安全機制**：警笛／通知 sonification 偵測自動降增益、啟動前安全聲明
- **Android Auto 整合**：AA 連線偵測、斷線自動停止、車載 Car App 介面；**偏好 USB 有線 AA**
- **雙音訊後端**：本機車庫驗證用 low-latency 路徑；AA 用 Track+submix
- **實車引導測試（僅路噪）**：
  - UI **只保留** `car_road_tuning_v1`（完整一般驗證 v3 已從 UI 移除）
  - 驗 **HIGH_LAT_PRED_BANK** / neuralLatent / imuMicCoherence / bankMatch* / lowBandRumbleReduction / 無靜電
  - **推進方式 = 有效行駛秒數**（`valid_drive_sec`）：車速達標才累計；**紅燈/怠速不計**；prep/finish 用短壁鐘
  - 使用者流程：**開始 → 開車 → 結束後存 log**（無需逐步按下一步）
...
1. `ANCService` 以前景服務啟動
2. **#8** 非 AA → `LocalLowLatencyAudio`（小 buffer + `PERFORMANCE_MODE_LOW_LATENCY`）；**AA** → 經典 `AudioRecord` / `AudioTrack` + submix（**不用** exclusive AAudio）
3. **#9** `AudioRouteManager`：`requireWiredAa`、USB/BUS 優先、BT 降權；`wirelessAaSuspected` 寫 log
4. 載入或執行車廂聲學校正（`CabinTransferModel`）；plant electrical delay = track + framework
5. `ReferenceSignalPipeline`：媒體扣除 + AEC + IMU rumble aux ref；高 lat rumble 時 micFactor→0
6. `MultiBandANCProcessor`：
   - 高 lat + rumble → `FF_PREVIEW_ONLY`（preview + road FF 主導）
   - **#6** delayless/partitioned FDAF（low）
   - **#7** PreLearned speed×roughness fixed FIR 為主、adaptive 為輔
7. 即時頻譜與 dB 透過 `AncStateManager`（StateFlow）回傳 UI

### 主要演算法模組

| 模組 | 說明 |
|------|------|
| `MultiBandANCProcessor` | 多頻帶 FxLMS；`FF_PREVIEW_ONLY` / driving rumble / MUSIC_DOMINANT_RUMBLE |
| `LatencyAwareBandLimiter` | **measured** 延遲限制中／高頻；high lat maxCancel ~60–120 Hz class |
| `RumblePreviewPredictor` | IMU+speed 外推 future rumble；horizon 隨 measured latency |
| `PreLearnedAncBank` | **#7** speed×roughness 2D 預訓 / 線上 capture；fixed FIR 主路徑 |
| `FdafLowBandProcessor` | **#6** 4 分區 + delayless FIR 輸出（low only） |
| `MultirateLowBandFxLms` | 低頻多速率 FxLMS |
| `EngineCombCanceller` | 引擎諧波前饋（手動 RPM；OBD 已移除） |
| `RoadNoiseWienerBank` | GPS 路噪 Wiener 前饋 |
| `VirtualSensingModel` | 虛擬感測 |
| `CabinMimoProfile` | 多區域 MIMO（試驗） |
| `SirenDetector` / `SonificationDetector` | 警笛／通知保護 |
| `ReferenceSignalPipeline` | 媒體扣除、AEC、IMU aux ref |
| `VehicleSpeedProvider` | GPS + IMU + **roughness**（給 #7 bank） |
| `LocalLowLatencyAudio` | **#8** 本機 low-latency 路徑 |
| `AudioRouteManager` | **#9** 有線 AA 偏好 / wireless 警示 |
...
## 關鍵 Log 欄位（2026-07-21 後）

| 欄位 | 意義 |
|------|------|
| `latencyStrategy` | `FF_PREVIEW_ONLY` / `HIGH_LAT_CONSERVATIVE` / `NORMAL` |
| `measuredLatencyMs` / `plantElectricalDelaySamples` | 真實路徑延遲與 plant 樣本 |
| `usingLatencyOverride` | 應為 `false`（ov 不驅動 plant） |
| `previewRumble` / `predictionHorizonMs` | 預測 FF 診斷 |
| `fdafDelayless` / `fdafPartitions` / `fixedBankOut` | #6/#7 路徑是否活躍 |
| `audioBackend` | `AAUDIO_LIKE_LOCAL_LOW_LATENCY` 或 `AUDIOTRACK_AA_SUBMIX` |
| `wirelessAaSuspected` / `wiredCarPathAvailable` | #9 有線/無線判斷 |
| `roadRoughness` / `preLearnedBinCount` | #7 bank 狀態 |

## 已知限制

- **音訊路由**：已強化 `AudioRouteManager`（USB 優先、AA 排除手機喇叭、wireless 警示）。部分 AA 裝置仍可能只暴露 submix；以 `wirelessAaSuspected`、`carSinkRouted` 驗證
- **延遲**：AA remote_submix 實測常 ~200–250+ ms，無法用 app 硬壓到 <50ms；靠 FF/IMU/preview/fixed bank
- **#8 AAudio**：真 exclusive AAudio 為 NDK API；本機用 Java low-latency Track/Record（多裝置底層走 AAudio）
- **#11 AAOS**：目前為 Car App **投影模板**，非車廠 ECU 級 RNC
- **Track buffer**：AA 路徑仍受 `getMinBufferSize()` / 系統 cap 影響
...
**Solution (`7f8107f`):** `GuidedTestController` advanceMode=`valid_drive_sec`

| Step | Need | Min speed |
|------|------|-----------|
| prep | wall 20s | — |
| #4 / #4b | 50s **valid** | ≥40 km/h |
| #5 | 45s valid | ≥40 |
| #6 | 50s valid | ≥45 |
| #7 | 55s valid | ≥50 |
| finish | wall 10s | — |

- UI: `有效數據 x/y` + `○ 暫停 · 車速不足` when stopped  
- Safety: `maxWallSec` ~10–12 min/step  
- Log: `test_step_complete` has `validSec`, `wallElapsedSec`, `advanceMode`  
- Related: `9c47d78` UI only road-tuning; `a2e84c3` first auto-advance; `7f8107f` valid-drive  
