...
## 最新進度（2026-07-21）— 請 pull 後讀這段

**Commit 系列（main）**：`dafbcac` (P0) → `b9e135c` (P1 preview 品質) → `f624ad0` (#6–#9 + #11)

| 編號 | 內容 | 狀態 |
|------|------|------|
| **P0** | 高延遲 `latencyStrategy=FF_PREVIEW_ONLY`：micFactor→0、LMS 弱/freeze、plant=measured track+fw、maxCancel 只用 measured（debug ov 僅 log） | 已做 |
| **P1** | `RumblePreviewPredictor` EMA/history 診斷；snapshot `previewRumble` / `predictionHorizonMs` | 已做 |
| **#6** | Delayless + partitioned FDAF（4×64，low only）；`fdafDelayless` / `fdafPartitions` | 已做 |
| **#7** | `PreLearnedAncBank` **speed×roughness** 2D；高 lat 下 fixed FIR 為主、adaptive 為輔；`setRoadRoughness` | 已做 |
| **#8** | **非 AA** 本機：`LocalLowLatencyAudio`（AAudio-like low latency）；**AA** 仍 AudioTrack + remote_submix | 已做 |
| **#9** | 有線 AA 優先：`requireWiredAa`、BT 降權、`wirelessAaSuspected` / `wiredCarPathAvailable` | 已做 |
| **#11** | Car App 模板 UI + optional automotive feature；**非**原廠 AAOS ECU RNC | scaffold |

**路測建議**：USB 有線 AA（非無線）；log 驗 `latencyStrategy` / `audioBackend` / `fdafDelayless` / `fixedBankOut` / `wirelessAaSuspected=false`。  
**安裝**：`.\scripts\install-debug.ps1` 或 `scripts\install-debug.bat`。  
...
- **實車引導測試**：
  ...
  - `car_road_tuning_v1`：路噪 rumble 快速迭代（#4b A/B + #6/#7；ov 僅 log；驗 FF_PREVIEW / fdafDelayless / fixedBank / USB有線AA）
