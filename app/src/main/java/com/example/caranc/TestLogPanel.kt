    ...
    Card(modifier = modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            if (isDevMode) {
                ...
                var personalBias = AncTestPreferences.getPersonalRumbleBias(context)
                if (personalBias < 1.15f) {
                    personalBias = 1.28f
                    AncTestPreferences.setPersonalRumbleBias(context, 1.28f)
                }
                Text("個人聲學偏好 (Personal Rumble Bias, 跟著你走; C15 rumble 推薦預設 1.28 top tier * personal): ${"%.2f".format(personalBias)} × (1.0=中性；>1 更強抗低頻 rumble)", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.secondary)
                Slider(
                    value = personalBias,
                    onValueChange = {
                ...
                Text("這是你的「聲學身分」：下次開任何車，只要同手機連 AA，偏好自動套用。C15: #7 rumble 設 1.28 + PRO + strict 55+ rough low-music 解鎖 IMU hybrid + crowd 1.5-1.8x (see c15_full_cycle11_report.txt)。", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
